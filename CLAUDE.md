...
- All displayed numbers go through `formatNumber` in `src/format.ts`: no thousands grouping, dot decimal, max 2 decimals, pinned to en-US, so the only on-screen separator is the decimal point. Never render a raw number or a bare `.toLocaleString()` in the UI.
